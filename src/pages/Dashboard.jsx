...
function ModalShell({ title, eyebrow, onClose, children, footer }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-950/45 p-3 backdrop-blur-[2px] sm:p-6" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div role="dialog" aria-modal="true" aria-labelledby="modal-title" className="max-h-[85vh] flex flex-col w-full max-w-2xl bg-[#faf9f6] rounded-2xl shadow-2xl overflow-hidden"><div className="p-5 border-b border-stone-200 flex justify-between items-center bg-[#faf9f6] shrink-0"><div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-stone-500">{eyebrow}</p><h2 id="modal-title" className="mt-1.5 font-serif text-2xl tracking-[-0.03em] text-stone-950 sm:text-3xl">{title}</h2></div><button type="button" aria-label="Tutup dialog" onClick={onClose} className="flex size-9 shrink-0 items-center justify-center rounded-full border border-[#E5E0D8] bg-white text-stone-500 transition hover:border-stone-400 hover:text-stone-900"><X aria-hidden="true" className="size-4" /></button></div><div className="p-6 overflow-y-auto flex-1 [&>form>div:last-child]:sticky [&>form>div:last-child]:bottom-[-1.5rem] [&>form>div:last-child]:bg-[#faf9f6]">{children}</div>{footer && <div className="p-4 border-t border-stone-200 bg-[#faf9f6] shrink-0 flex items-center justify-between">{footer}</div>}</div></div>;
}
