...
function BookCover({ book, large = false }) {
  return <div className={`relative aspect-[3/4] w-full overflow-hidden bg-gradient-to-br ${book.cover || 'from-stone-300 to-stone-600'} ${large ? 'max-w-[280px]' : ''}`}>
    ...
    {!book.coverUrl && <div className="absolute inset-x-5 bottom-5 border-l border-white/50 pl-3 text-white"><p className="font-serif text-3xl leading-none tracking-[-0.05em]">{book.initials || 'BK'}</p><p className="mt-2 max-w-[120px] text-[9px] uppercase tracking-[0.16em] text-white/75">Koleksi buku</p></div>}
  </div>;
}
...
function AddBookModal({ onClose, onSave, book = null }) {
  const [form, setForm] = useState(() => book ? { judul: book.judul || '', penulis: book.penulis || '', penerbit: book.penerbit || '', genre: book.genre || '', stok: String(book.stok || 0), coverUrl: book.coverUrl || '', description: book.description || book.deskripsi || '' } : { judul: '', penulis: '', penerbit: '', genre: '', stok: '0', coverUrl: '', description: '' });
...
function BookDetailModal({ book, onClose, onBorrow, onEdit }) {
  ...
  if (!book) return null;
  const description = book.description || book.deskripsi || 'Deskripsi buku belum tersedia.';
  const available = Number(book.stok) > 0 || book.status === 'Tersedia';
  const specifications = [['ISBN', book.isbn || book.ISBN || '-'], ['Tanggal terbit', book.tanggalTerbit || book.tanggal_terbit || '-'], ['Jumlah halaman', book.halaman ? `${book.halaman} halaman` : '-'], ['Bahasa', book.bahasa || 'Indonesia'], ['Stok tersedia', `${book.stok || 0} buku`]];
  return <ModalShell eyebrow="Detail koleksi" title={book.judul} onClose={onClose}><div className="grid gap-7 p-5 sm:grid-cols-[minmax(150px,0.72fr)_1.28fr] sm:p-7"><div><div className="overflow-hidden rounded-lg shadow-[0_12px_28px_rgba(74,59,45,0.12)]"><BookCover book={book} large /></div><div className="mt-4 flex flex-col gap-2"><button type="button" disabled={!available} onClick={() => onBorrow(book)} className="rounded-md bg-stone-900 px-4 py-3 text-xs font-semibold text-[#FAF7F2] transition hover:bg-stone-800 disabled:cursor-not-allowed disabled:bg-stone-300">{available ? 'Pinjam Buku' : 'Stok Habis'}</button><button type="button" onClick={() => onEdit(book)} className="rounded-md border border-stone-300 px-4 py-3 text-xs font-semibold text-stone-700 transition hover:bg-stone-50">Edit Data</button></div></div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-[#f4eee6] px-2.5 py-1 text-[10px] font-medium text-[#876a4c]">{book.genre || 'Tanpa genre'}</span><StatusBadge tone={available ? 'green' : 'stone'}>{available ? 'Tersedia' : 'Dipinjam'}</StatusBadge></div><h3 className="mt-3 font-serif text-3xl leading-tight tracking-[-0.04em] text-stone-900">{book.judul}</h3><p className="mt-2 text-sm text-stone-500">{book.penulis} <span className="text-stone-300">·</span> {book.penerbit || 'Penerbit tidak tersedia'}</p><section className="mt-6 border-t border-stone-200 pt-5"><h4 className="font-serif text-lg text-stone-900">Deskripsi Buku</h4><p className={`mt-2 text-sm leading-6 text-stone-500 ${expanded ? '' : 'line-clamp-3'}`}>{description}</p>{description.length > 170 && <button type="button" onClick={() => setExpanded((current) => !current)} className="mt-2 text-xs font-semibold text-stone-800 underline underline-offset-4">{expanded ? 'Tampilkan lebih sedikit' : 'Baca selengkapnya'}</button>}</section><section className="mt-6 border-t border-stone-200 pt-5"><h4 className="font-serif text-lg text-stone-900">Detail Spesifikasi</h4><dl className="mt-3 grid gap-2.5 text-sm">{specifications.map(([label, value]) => <div key={label} className="flex items-center justify-between gap-4 border-b border-stone-100 pb-2.5"><dt className="text-stone-500">{label}</dt><dd className="text-right font-medium text-stone-800">{value}</dd></div>)}</dl></section></div></div><div className="flex justify-end border-t border-stone-200 px-5 py-4 sm:px-7"><button type="button" onClick={onClose} className="rounded-md bg-stone-900 px-4 py-2.5 text-xs font-medium text-[#FAF7F2] hover:bg-stone-800">Tutup</button></div></ModalShell>;
}
...
export default function Dashboard() {
  ...
  const handleEditBook = (form) => {
    setBooks((current) => current.map((book) => book.id_buku === detailBook?.id_buku ? { ...book, ...form, stok: Number(form.stok), status: Number(form.stok) > 0 ? 'Tersedia' : 'Dipinjam', initials: getInitials(form.judul.trim()) } : book));
