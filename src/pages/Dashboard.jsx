...
function DeleteBookModal({ book, onClose, onConfirm }) {
  if (!book) return null;
  return <ModalShell eyebrow="Koleksi" title="Hapus buku" onClose={onClose}><div className="space-y-4 p-5 sm:p-8"><p className="text-sm leading-6 text-stone-600">Apakah Anda yakin ingin menghapus buku ini?</p><p className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 font-medium text-red-800">{book.judul}</p></div><div className="flex justify-end gap-3 border-t border-[#E5E0D8] px-5 py-4 sm:px-8"><button type="button" onClick={onClose} className="rounded-md border border-stone-300 px-4 py-2.5 text-xs font-medium text-stone-700 hover:bg-white">Batal</button><button type="button" onClick={onConfirm} className="rounded-md bg-red-700 px-4 py-2.5 text-xs font-semibold text-white hover:bg-red-800">Ya, Hapus</button></div></ModalShell>;
}
