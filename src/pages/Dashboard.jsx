...
function ModalShell({ title, eyebrow, onClose, children, footer }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-950/45 p-3 backdrop-blur-[2px] sm:p-6" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div role="dialog" aria-modal="true" aria-labelledby="modal-title" className="relative flex flex-col w-full max-w-2xl max-h-[85vh] bg-[#faf9f6] rounded-2xl shadow-2xl overflow-hidden"><div className="flex items-center justify-between px-6 py-4 border-b border-stone-200 bg-[#faf9f6] shrink-0"><div><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-stone-500">{eyebrow}</p><h2 id="modal-title" className="mt-1.5 font-serif text-2xl tracking-[-0.03em] text-stone-950 sm:text-3xl">{title}</h2></div><button type="button" aria-label="Tutup dialog" onClick={onClose} className="flex size-9 shrink-0 items-center justify-center rounded-full border border-[#E5E0D8] bg-white text-stone-500 transition hover:border-stone-400 hover:text-stone-900"><X aria-hidden="true" className="size-4" /></button></div><div className="flex-1 overflow-y-auto p-6 space-y-4">{children}</div>{footer && <div className="flex items-center justify-between px-6 py-4 border-t border-stone-200 bg-[#faf9f6] shrink-0">{footer}</div>}</div></div>;
}
function DeleteBookModal({ book, onClose, onConfirm }) {
  if (!book) return null;
  return <ModalShell eyebrow="Koleksi" title="Hapus buku" onClose={onClose}><div className="space-y-4 p-6 pb-6 sm:p-8"><p className="text-sm leading-6 text-stone-600">Apakah Anda yakin ingin menghapus buku ini?</p><p className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 font-medium text-red-800">{book.judul}</p></div><div className="mt-6 flex justify-end gap-3 border-t border-stone-100 px-6 pt-4 pb-6 sm:px-8"><button type="button" onClick={onClose} className="rounded-lg border border-stone-300 px-4 py-2 text-sm font-medium text-stone-700 transition hover:bg-stone-100">Batal</button><button type="button" onClick={onConfirm} className="rounded-md bg-red-700 px-4 py-2.5 text-xs font-semibold text-white hover:bg-red-800">Ya, Hapus</button></div></ModalShell>;
}
