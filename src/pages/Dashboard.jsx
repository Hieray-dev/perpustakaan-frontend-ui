...
function readLoanData() {
  return parseLocalArray(LOANS_STORAGE_KEY).map(normalizeLoan).filter(Boolean);
}
...
function ShiftModal({ member, onClose, onSave }) {
  const [shift, setShift] = useState(member.shift === 'Belum diatur' ? '' : member.shift);
  const [customMode, setCustomMode] = useState(member.shift !== 'Belum diatur' && !['Pagi · 08.00–16.00', 'Siang · 12.00–20.00'].includes(member.shift));
  const inputClass = 'h-10 w-full rounded-md border border-stone-200 bg-stone-50 px-3 text-sm text-stone-900 outline-none focus:border-stone-400 focus:ring-2 focus:ring-stone-200';
  const submit = (event) => { event.preventDefault(); if (shift.trim()) onSave(shift.trim()); };
  return <ModalShell eyebrow="Administrasi" title={`Atur shift · ${member.name}`} onClose={onClose}><form onSubmit={submit}><div className="flex flex-col gap-4 p-5"><label><span className="mb-1.5 block text-xs font-medium text-stone-700">Pilihan shift</span><select value={customMode ? '__custom__' : shift} onChange={(event) => { const value = event.target.value; setCustomMode(value === '__custom__'); setShift(value === '__custom__' ? '' : value); }} className={inputClass}><option value="">Belum diatur</option><option value="Pagi · 08.00–16.00">Pagi: 08.00 - 16.00</option><option value="Siang · 12.00–20.00">Siang: 12.00 - 20.00</option><option value="__custom__">Jam kustom</option></select></label><label><span className="mb-1.5 block text-xs font-medium text-stone-700">Jadwal tersimpan</span><input required name="shift" value={shift} onChange={(event) => { setCustomMode(true); setShift(event.target.value); }} placeholder="Contoh: Malam · 20.00–04.00" className={inputClass} /><span className="mt-1.5 block text-xs text-stone-500">Jadwal ini akan dipakai sebagai acuan waktu masuk pada widget absensi.</span></label></div><div className="flex justify-end gap-3 border-t border-stone-200 px-5 py-4"><button type="button" onClick={onClose} className="rounded-md border border-stone-300 px-4 py-2.5 text-xs font-medium text-stone-700 hover:bg-stone-50">Batal</button><button type="submit" className="rounded-md bg-stone-900 px-4 py-2.5 text-xs font-medium text-[#FAF7F2] hover:bg-stone-800">Simpan shift</button></div></form></ModalShell>;
}

function StaffManagement({ staff, onAddStaff, onEditShift }) {
  const [selectedMember, setSelectedMember] = useState(null);
  return <><PageHeader eyebrow="Administrasi" title="Karyawan & shift" description="Data pengguna internal, peran, dan jadwal shift." action={<button type="button" onClick={onAddStaff} className="flex items-center justify-center gap-2 rounded-md bg-stone-900 px-4 py-3 text-xs font-semibold text-[#FAF7F2] hover:bg-stone-800"><Plus aria-hidden="true" className="size-4" /> Tambah karyawan</button>} /><section className="overflow-hidden rounded-lg border border-stone-200 bg-white"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-stone-50 text-[11px] text-stone-500"><tr><th className="px-6 py-3 font-medium">Nama</th><th className="px-5 py-3 font-medium">Username</th><th className="px-5 py-3 font-medium">Peran</th><th className="px-5 py-3 font-medium">Jadwal shift</th><th className="px-5 py-3 font-medium">Status</th><th className="px-5 py-3 font-medium">Aksi</th></tr></thead><tbody className="divide-y divide-stone-100">{staff.length ? staff.map((member) => <tr key={member.id}><td className="px-6 py-4 font-medium text-stone-800">{member.name}</td><td className="px-5 py-4 text-stone-600">@{member.username}</td><td className="px-5 py-4 text-stone-600">{member.role}</td><td className="px-5 py-4 text-stone-600">{member.shift}</td><td className="px-5 py-4"><StatusBadge tone="success">{member.status}</StatusBadge></td><td className="px-5 py-4"><button type="button" onClick={() => setSelectedMember(member)} className="rounded-md border border-stone-200 px-2.5 py-1.5 text-[11px] font-medium text-stone-600 hover:border-stone-400 hover:bg-stone-50">{member.shift === 'Belum diatur' ? 'Atur shift' : 'Ubah shift'}</button></td></tr>) : <tr><td colSpan="6" className="px-6 py-12 text-center text-sm text-stone-500">Belum ada data karyawan/absensi</td></tr>}</tbody></table></div></section>{selectedMember && <ShiftModal member={selectedMember} onClose={() => setSelectedMember(null)} onSave={(shift) => { onEditShift(selectedMember, shift); setSelectedMember(null); }} />}</>;
}
...
export default function Dashboard() {
  ...
  const handleBooking = (book) => {
    const newLoan = { id: Date.now(), code: `TRX-${new Date().getFullYear()}-${String(loans.length + 1).padStart(4, '0')}`, member: getDisplayName(user), book: book.judul, date: formatTransactionDate(), due: '28 Sep 2026', status: 'Dipinjam', fine: 0 };
    const nextLoans = [newLoan, ...loans];
    setLoans(nextLoans);
    writeLoanData(nextLoans);
  };
